...
async function importTennesseeData() {
  console.log('Starting Tennessee facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('tennessee_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Tennessee facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('TENNESSEE IMPORT SUMMARY');
...
importTennesseeData().catch(console.error);