...
async function importRhodeIslandData() {
  console.log('Starting Rhode Island facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('rhode_island_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Rhode Island facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('RHODE ISLAND IMPORT SUMMARY');
...
importRhodeIslandData().catch(console.error);