  ...
  app.post('/api/service-intelligence', async (req, res) => {
    try {
      ...
      try {
        ...
        console.error('Cache enrichment failed, falling back to Search API:', cacheError);
        // Continue with the Search API fallback below
      }

      // MIGRATED: Use Perplexity Search API instead of Sonar Pro
      // Cost: $0.005 per request vs ~$0.02-0.10+ for Sonar Pro
      const { perplexitySearchAPI } = await import('./services/perplexity-search-api');
      
      try {
        const searchResult = await perplexitySearchAPI.searchBusinessIntelligence(
          serviceName,
          location || `${city}, ${state}`,
        ...
        if (searchResult.found) {
          return res.json({
            ...
            website: searchResult.data.website || website || '',
            photos: searchResult.photos.map(url => {
              if (url.includes('http') && !url.includes('/api/image-proxy')) {
                return `/api/image-proxy?url=${encodeURIComponent(url)}`;
              }
              return url;
        ...
        console.error('Search API fallback failed:', searchApiError);
      }

      // If Search API also fails, return empty result
      return res.json({
        success: false,
        serviceName,
        location: `${city}, ${state}`,
