...
/**
 * Communities search endpoint - GET version
 * Handles community searches with simple query parameters
 */
router.get('/api/communities/search', async (req, res) => {
  try {
    ...
    // Use comprehensive search engine for community search
    const results = await comprehensiveSearchEngine.search(
      searchQuery,
      filters,
      { limit: parseInt(limit as string), offset: parseInt(offset as string) }
    ...
    res.json({
      ...
      _version: 'communities_search_v1',
      _timestamp: Date.now()
    });
...
/**
 * Communities search endpoint - POST version
 * Handles community searches with JSON body
 */
router.post('/api/communities/search', async (req, res) => {
  try {
    const { 
      query = '', 
      filters = {},
