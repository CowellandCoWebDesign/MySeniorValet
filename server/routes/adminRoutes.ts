...
export function registerAdminRoutes(app: Express) {
  ...
  adminRouter.get('/theme-settings', async (req, res) => {
    try {
      // Get theme settings from database or use defaults
      ...
      res.json({
        theme: JSON.parse(theme.setting_value as string),
        presets: [
          { id: 'default', name: 'Cosmic Night', primary: 'hsl(222, 47%, 11%)' },
          { id: 'light', name: 'Clean Light', primary: 'hsl(0, 0%, 100%)' },
          { id: 'ocean', name: 'Ocean Blue', primary: 'hsl(210, 80%, 50%)' },
          { id: 'forest', name: 'Forest Green', primary: 'hsl(140, 60%, 40%)' }
        ]
      });
    } catch (error) {
  ...
  adminRouter.post('/theme-settings', async (req, res) => {
    try {
      ...
      await db.execute(sql`
        ...
        VALUES ('theme', ${JSON.stringify(themeData)}, NOW())
