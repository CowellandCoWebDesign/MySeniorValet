...
export class MultiAIPhotoExtractor {
  /**
   * Main extraction method for services/businesses (instance method)
   * This is what the route expects to call
   */
  async extractPhotos(
    serviceName: string, 
    searchQuery: string, 
    ...
    try {
      // Call Perplexity to get web content about the business
      const perplexityApiKey = process.env.PERPLEXITY_API_KEY;
      if (!perplexityApiKey) {
        console.error('Perplexity API key not configured');
        return { photos: [], sources: [], confidence: 0 };
      }
      
      console.log(`🔍 Extracting photos for: ${serviceName} (${serviceType || 'business'})`);
      
      // First, use Perplexity to find the business website and information
      const perplexityPrompt = `Find information about "${serviceName}" ${searchQuery}. 
      
Provide:
1. Their official website URL
2. Google Maps listing URL
3. Yelp page URL
4. Facebook page URL
5. Any other directory listings with photos
6. Direct links to any photo galleries or images you find
...
Include actual URLs in your response.`;
      ...
      const perplexityResponse = await fetch('https://api.perplexity.ai/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a business information specialist. Provide actual URLs and links to business listings.'
            },
            {
              role: 'user',
      ...
      const websitePattern = /(?:website|Website|Official site).*?(https?:\/\/[^\s,\)]+)/gi;
      const websiteMatches = [...content.matchAll(websitePattern)];
      let officialWebsite = websiteMatches[0]?.[1];
      
      // Also check citations for the actual business website
      if (!officialWebsite && citations.length > 0) {
        for (const citation of citations) {
          const url = citation.url || citation;
          if (typeof url === 'string' && url.includes(serviceName.toLowerCase().replace(/\s+/g, ''))) {
            officialWebsite = url;
            break;
          }
        }
      ...
      // Extract all photos we can find
      let allPhotoUrls: string[] = [];
      
      // Method 1: Extract from Perplexity content
      const contentPhotos = MultiAIPhotoExtractor.extractPhotosFromContent(content, serviceName);
      allPhotoUrls.push(...contentPhotos.map(p => p.url));
      
      // Method 2: Scrape official website with Playwright if we have one
      if (officialWebsite) {
        console.log(`🌐 Found official website: ${officialWebsite}`);
        console.log('🎭 Using Playwright to scrape actual photos from website...');
        ...
        try {
          ...
          console.log(`✅ Scraped ${scrapedPhotos.length} photos from ${officialWebsite}`);
          allPhotoUrls.push(...scrapedPhotos.map(p => p.url));
        } catch (scrapeError) {
          console.error('Failed to scrape website:', scrapeError);
        }
      }
      
      // Method 3: Try to extract from directory sites mentioned
      const directoryPhotos = await MultiAIPhotoExtractor.extractPhotosFromServiceDirectorySites(content, serviceName, serviceType);
      allPhotoUrls.push(...directoryPhotos.map(p => p.url));
      
      // Method 4: If still no photos, try generic image search patterns
      if (allPhotoUrls.length === 0) {
        console.log('🔍 Attempting fallback photo extraction...');
        
        // Try to find any image URLs in citations
        for (const citation of citations) {
          const url = citation.url || citation;
          if (typeof url === 'string') {
            // Check if citation itself might be an image
            if (/\.(jpg|jpeg|png|gif|webp)/i.test(url)) {
              allPhotoUrls.push(url);
            }
            // Or try to scrape the citation page
            else if (!url.includes('zoominfo') && !url.includes('linkedin')) {
              try {
                const citationPhotos = await playwrightPhotoScraper.scrapePhotosFromWebsite(url, serviceName);
                if (citationPhotos.length > 0) {
                  console.log(`✅ Found ${citationPhotos.length} photos from ${new URL(url).hostname}`);
                  allPhotoUrls.push(...citationPhotos.map(p => p.url));
                  break; // Stop after finding photos from one source
                }
              } catch (e) {
                // Silently continue
              }
      ...
      // Remove duplicates
      const uniquePhotoUrls = [...new Set(allPhotoUrls)];
      
      // Extract source domains from citations
      const sources = citations.map((c: any) => {
        try {
          const url = new URL(c.url || c);
      ...
      console.log(`✅ Found ${uniquePhotoUrls.length} unique photos for ${serviceName}`);
      
      return {
        photos: uniquePhotoUrls,
        sources: sources,
        confidence: uniquePhotoUrls.length > 0 ? 0.8 : 0,
        businessInfo: {
      ...
      console.error(`Failed to extract photos for ${serviceName}:`, error);
      return { photos: [], sources: [], confidence: 0 };
    }
  }
