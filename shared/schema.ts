...
// Predictive Models - AI/ML models for community predictions
export const predictiveModels = pgTable("predictive_models", {
  id: serial("id").primaryKey(),
  communityId: integer("community_id").references(() => communities.id),
  modelType: varchar("model_type", { length: 100 }).notNull(), // 'occupancy_forecast', 'risk_assessment', 'maintenance_prediction'
  ...
  isActive: boolean("is_active").default(true),
  modelData: jsonb("model_data"), // Serialized model parameters
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// AI Insights - Automated insights generated by AI systems
export const aiInsights = pgTable("ai_insights", {
  id: serial("id").primaryKey(),
  communityId: integer("community_id").references(() => communities.id),
  insightType: varchar("insight_type", { length: 100 }).notNull(), // 'census_prediction', 'risk_alert', 'revenue_opportunity'
  title: varchar("title", { length: 255 }).notNull(),
  description: text("description").notNull(),
...
// Occupancy Forecasts - ML-generated occupancy predictions
export const occupancyForecasts = pgTable("occupancy_forecasts", {
  id: serial("id").primaryKey(),
  communityId: integer("community_id").references(() => communities.id),
  forecastDate: date("forecast_date").notNull(),
  predictedOccupancy: decimal("predicted_occupancy", { precision: 5, scale: 2 }).notNull(),
  confidence: decimal("confidence", { precision: 5, scale: 2 }).notNull(),
  actualOccupancy: decimal("actual_occupancy", { precision: 5, scale: 2 }),
...
export const anomalyDetections = pgTable("anomaly_detections", {
  id: serial("id").primaryKey(),
  communityId: integer("community_id").references(() => communities.id),
  anomalyType: varchar("anomaly_type", { length: 100 }).notNull(), // 'occupancy_drop', 'cost_spike', 'maintenance_overdue'
  severity: varchar("severity", { length: 50 }).notNull(),
  detectedValue: decimal("detected_value", { precision: 10, scale: 2 }).notNull(),
  expectedValue: decimal("expected_value", { precision: 10, scale: 2 }).notNull(),
...
// AI Reports - Natural language reports generated by AI
export const aiReports = pgTable("ai_reports", {
  id: serial("id").primaryKey(),
  communityId: integer("community_id").references(() => communities.id),
  reportType: varchar("report_type", { length: 100 }).notNull(), // 'weekly_summary', 'monthly_analysis', 'quarterly_forecast'
  title: varchar("title", { length: 255 }).notNull(),
  summary: text("summary").notNull(),
  content: text("content").notNull(), // Natural language generated content
  keyInsights: jsonb("key_insights"), // Structured key insights
  recommendations: jsonb("recommendations"), // AI-generated recommendations
  metrics: jsonb("metrics"), // Relevant metrics and KPIs
  generatedAt: timestamp("generated_at").defaultNow(),
  periodStart: date("period_start").notNull(),
  periodEnd: date("period_end").notNull(),
});

// Staff Optimization Analytics - AI-driven staff scheduling insights
export const staffOptimization = pgTable("staff_optimization", {
  id: serial("id").primaryKey(),
  communityId: integer("community_id").references(() => communities.id),
  analysisDate: date("analysis_date").notNull(),
  recommendedStaffLevels: jsonb("recommended_staff_levels"), // Staff levels by role and shift
  costOptimization: jsonb("cost_optimization"), // Cost-saving opportunities
  efficiencyMetrics: jsonb("efficiency_metrics"), // Productivity and efficiency scores
  predictedWorkload: jsonb("predicted_workload"), // Forecasted workload requirements
