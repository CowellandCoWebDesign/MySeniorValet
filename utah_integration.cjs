...
async function importUtahData() {
  console.log('Starting Utah facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('utah_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Utah facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('UTAH IMPORT SUMMARY');
...
importUtahData().catch(console.error);