...
async function importMississippiData() {
  console.log('Starting Mississippi facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('mississippi_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Mississippi facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('MISSISSIPPI IMPORT SUMMARY');
...
importMississippiData().catch(console.error);