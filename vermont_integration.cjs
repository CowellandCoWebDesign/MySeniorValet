...
async function importVermontData() {
  console.log('Starting Vermont facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('vermont_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Vermont facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('VERMONT IMPORT SUMMARY');
...
importVermontData().catch(console.error);