...
async function importArkansasData() {
  console.log('Starting Arkansas facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('arkansas_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Arkansas facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('ARKANSAS IMPORT SUMMARY');
...
importArkansasData().catch(console.error);