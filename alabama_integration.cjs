...
async function importAlabamaData() {
  console.log('Starting Alabama facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('alabama_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Alabama facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('ALABAMA IMPORT SUMMARY');
...
importAlabamaData().catch(console.error);