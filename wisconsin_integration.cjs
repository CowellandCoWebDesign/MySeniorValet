...
async function importWisconsinData() {
  console.log('Starting Wisconsin facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('wisconsin_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Wisconsin facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('WISCONSIN IMPORT SUMMARY');
...
importWisconsinData().catch(console.error);