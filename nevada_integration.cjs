...
async function importNevadaData() {
  console.log('Starting Nevada facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('nevada_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Nevada facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('NEVADA IMPORT SUMMARY');
...
importNevadaData().catch(console.error);