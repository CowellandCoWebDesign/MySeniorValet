...
async function importOklahomaData() {
  console.log('Starting Oklahoma facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('oklahoma_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Oklahoma facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('OKLAHOMA IMPORT SUMMARY');
...
importOklahomaData().catch(console.error);