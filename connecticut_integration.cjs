...
async function importConnecticutData() {
  console.log('Starting Connecticut facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('connecticut_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Connecticut facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('CONNECTICUT IMPORT SUMMARY');
...
importConnecticutData().catch(console.error);