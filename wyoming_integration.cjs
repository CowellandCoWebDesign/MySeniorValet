...
async function importWyomingData() {
  console.log('Starting Wyoming facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('wyoming_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Wyoming facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('WYOMING IMPORT SUMMARY');
...
importWyomingData().catch(console.error);