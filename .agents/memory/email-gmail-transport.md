...
as the Workspace account `hello@myseniorvalet.com`. SendGrid is no longer used for
delivery even though `@sendgrid/mail` is still imported everywhere.
...
quarantined (Workspace spoofing, e.g. contact admin notice sent from==to hello@→hello@).
...
- Gmail can only send AS the authenticated account. `From` always uses
  `hello@myseniorvalet.com`; a caller's differing `from` is preserved as `Reply-To`.
