...
async function importArizonaData() {
  console.log('Starting Arizona facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('arizona_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Arizona facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('ARIZONA IMPORT SUMMARY');
...
importArizonaData().catch(console.error);