...
async function importIndianaData() {
  console.log('Starting Indiana facilities data import...\n');
  
  // Connect to database
  const sql = neon(process.env.DATABASE_URL);
  
  // Find the latest Indiana facilities file
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('indiana_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Indiana facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('INDIANA IMPORT SUMMARY');
...
// Run the import
importIndianaData().catch(console.error);