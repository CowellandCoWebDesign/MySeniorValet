...
- **DESIGN COLOR PRESERVATION POLICY** (January 13, 2025): User's purple/pink gradient aesthetic must be preserved:
  - NEVER change design colors from purple/pink gradients - user loves this color scheme
  - Only fix TEXT contrast for light mode readability (use gray-700 for text, keep purple for design)
  - Purple gradients for buttons, sections, and decorative elements must remain
  - Text contrast fixes: purple text → gray-700 in light mode, purple-400 in dark mode
  - The purple/pink/blue gradient aesthetic is core to the brand identity
