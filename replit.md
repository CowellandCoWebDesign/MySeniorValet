...
**MAP FUNCTIONALITY CRITICAL ISSUE**: Map panning/re-searching bug causing extreme user frustration. Communities don't load when map is panned to new areas (e.g., LA shows 97 communities but panning to Beverly Hills shows none). The map bounds are updating but queries aren't refetching. User requirement: "I need REAL data here. No more hardcoded estimates" and specifically stated "we have to find a way to fix this damn map". Added debugging and working on proper query invalidation. San Francisco is now the default map location when no user location is provided. (January 19, 2025)
