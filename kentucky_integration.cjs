...
async function importKentuckyData() {
  console.log('Starting Kentucky facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('kentucky_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Kentucky facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('KENTUCKY IMPORT SUMMARY');
...
importKentuckyData().catch(console.error);