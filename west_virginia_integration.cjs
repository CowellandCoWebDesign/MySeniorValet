...
async function importWestVirginiaData() {
  console.log('Starting West Virginia facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('west_virginia_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No West Virginia facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('WEST VIRGINIA IMPORT SUMMARY');
...
importWestVirginiaData().catch(console.error);