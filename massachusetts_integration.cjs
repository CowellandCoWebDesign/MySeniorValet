...
async function importMassachusettsData() {
  console.log('Starting Massachusetts facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('massachusetts_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Massachusetts facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('MASSACHUSETTS IMPORT SUMMARY');
...
importMassachusettsData().catch(console.error);