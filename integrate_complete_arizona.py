...
def read_arizona_facilities(csv_file):
    """Read Arizona facilities from CSV file"""
    facilities = []
    try:
...
def create_sql_inserts(facilities, batch_size=20):
    ...
    for batch_num, batch in enumerate(batches, 1):
        ...
        for facility in batch:
            ...
            value = f"('{name}', '{address}', '{city}', 'AZ', '{facility['zipCode']}', '{phone}', ARRAY['Assisted Living', 'Memory Care'], '{county}', 'arizona_government_records', NOW(), NOW())"
            values.append(value)
...
def save_sql_file(sql_statements, filename="arizona_complete_integration.sql"):
    """Save SQL statements to file"""
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("-- Complete Arizona Expansion - 252 Facilities\n")
            f.write("-- Generated: " + datetime.now().isoformat() + "\n")
            f.write("-- Source: Arizona Department of Health Services\n")
            f.write("-- Coverage: All 15 Arizona counties\n\n")
...
def main():
    ...
    csv_file = "arizona_complete_facilities_20250710_110235.csv"
    facilities = read_arizona_facilities(csv_file)
    ...
    if sql_statements:
        ...
        if sql_file:
            print(f"\n🎉 Complete Arizona Integration Ready!")
