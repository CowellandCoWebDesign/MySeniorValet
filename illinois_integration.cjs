...
async function importIllinoisData() {
  console.log('Starting Illinois facilities data import...\n');
  
  // Connect to database
  const sql = neon(process.env.DATABASE_URL);
  
  // Find the latest Illinois facilities file
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('illinois_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Illinois facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('ILLINOIS IMPORT SUMMARY');
...
// Run the import
importIllinoisData().catch(console.error);