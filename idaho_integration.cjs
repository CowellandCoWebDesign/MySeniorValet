...
async function importIdahoData() {
  console.log('Starting Idaho facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('idaho_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Idaho facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('IDAHO IMPORT SUMMARY');
...
importIdahoData().catch(console.error);