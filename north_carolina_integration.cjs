...
async function importNorthCarolinaData() {
  console.log('Starting North Carolina facilities data import...\n');
  
  // Connect to database
  const sql = neon(process.env.DATABASE_URL);
  
  // Find the latest North Carolina facilities file
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('north_carolina_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No North Carolina facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('NORTH CAROLINA IMPORT SUMMARY');
...
// Run the import
importNorthCarolinaData().catch(console.error);