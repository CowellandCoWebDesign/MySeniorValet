...
async function importColoradoData() {
  console.log('Starting Colorado facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('colorado_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Colorado facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('COLORADO IMPORT SUMMARY');
...
importColoradoData().catch(console.error);