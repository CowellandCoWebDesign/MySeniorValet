...
async function importNewHampshireData() {
  console.log('Starting New Hampshire facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('new_hampshire_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No New Hampshire facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('NEW HAMPSHIRE IMPORT SUMMARY');
...
importNewHampshireData().catch(console.error);