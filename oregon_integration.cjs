...
async function importOregonData() {
  console.log('Starting Oregon facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('oregon_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Oregon facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('OREGON IMPORT SUMMARY');
...
importOregonData().catch(console.error);