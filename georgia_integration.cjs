...
async function importGeorgiaData() {
  console.log('Starting Georgia facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('georgia_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Georgia facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('GEORGIA IMPORT SUMMARY');
...
importGeorgiaData().catch(console.error);