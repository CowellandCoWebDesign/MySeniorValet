...
async function importSouthCarolinaData() {
  console.log('Starting South Carolina facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('south_carolina_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No South Carolina facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('SOUTH CAROLINA IMPORT SUMMARY');
...
importSouthCarolinaData().catch(console.error);