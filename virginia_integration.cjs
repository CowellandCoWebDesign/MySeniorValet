...
async function importVirginiaData() {
  console.log('Starting Virginia facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('virginia_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Virginia facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('VIRGINIA IMPORT SUMMARY');
...
importVirginiaData().catch(console.error);