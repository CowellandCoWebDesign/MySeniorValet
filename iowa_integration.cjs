...
async function importIowaData() {
  console.log('Starting Iowa facilities data import...\n');
  
  // Connect to database
  const sql = neon(process.env.DATABASE_URL);
  
  // Find the latest Iowa facilities file
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('iowa_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Iowa facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('IOWA IMPORT SUMMARY');
...
// Run the import
importIowaData().catch(console.error);