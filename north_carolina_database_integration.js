...
async function integrateNorthCarolinaData() {
  try {
    console.log('🏔️ NORTH CAROLINA DATABASE INTEGRATION STARTING...');
    
    // Load North Carolina data
    const ncData = JSON.parse(fs.readFileSync('north_carolina_complete_facilities_20250717_145633.json', 'utf8'));
    console.log(`📊 Loaded ${ncData.length} North Carolina facilities`);
    
    let insertedCount = 0;
    let skippedCount = 0;
    
    // Process each facility
    for (const facility of ncData) {
      try {
    ...
    console.log(`\n🎉 NORTH CAROLINA INTEGRATION COMPLETE!`);
    console.log(`✅ Inserted: ${insertedCount} facilities`);
    console.log(`⏭️  Skipped: ${skippedCount} facilities (already exist)`);
    
    // Verify North Carolina data in database
    ...
    console.log(`\n🏔️ NORTH CAROLINA DATABASE VERIFICATION:`);
    console.log(`📊 Total facilities: ${ncResult.rows[0].total_facilities}`);
    console.log(`🏘️  Counties covered: ${ncResult.rows[0].counties_covered}`);
    console.log(`🏙️  Cities covered: ${ncResult.rows[0].cities_covered}`);
    
    // Show North Carolina county breakdown
    const ncCounties = await pool.query(`
      SELECT county, COUNT(*) as count
      FROM communities 
      WHERE state = 'NC' AND county IS NOT NULL
      GROUP BY county 
      ORDER BY count DESC
      LIMIT 15
    `);
    
    console.log(`\n🏔️ TOP NORTH CAROLINA COUNTIES:`);
    ncCounties.rows.forEach((row, index) => {
      console.log(`  ${index + 1}. ${row.county}: ${row.count} facilities`);
    });
    
    // Show North Carolina major metro coverage
    const ncMetros = await pool.query(`
      SELECT city, COUNT(*) as count
      FROM communities 
      WHERE state = 'NC' AND city IN ('Charlotte', 'Raleigh', 'Greensboro', 'Winston-Salem', 'Durham', 'Fayetteville', 'Asheville', 'Wilmington')
      GROUP BY city 
      ORDER BY count DESC
    `);
    
    console.log(`\n🏙️ NORTH CAROLINA MAJOR METRO COVERAGE:`);
    ncMetros.rows.forEach((row, index) => {
    ...
    console.error('❌ North Carolina integration failed:', error);
  } finally {
    await pool.end();
  }
}
...
integrateNorthCarolinaData().catch(console.error);