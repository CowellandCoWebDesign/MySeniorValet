...
async function importMichiganData() {
  console.log('Starting Michigan facilities data import...\n');
  
  // Connect to database
  const sql = neon(process.env.DATABASE_URL);
  
  // Find the latest Michigan facilities file
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('michigan_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Michigan facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('MICHIGAN IMPORT SUMMARY');
...
// Run the import
importMichiganData().catch(console.error);