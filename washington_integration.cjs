...
async function importWashingtonData() {
  console.log('Starting Washington facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('washington_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Washington facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('WASHINGTON IMPORT SUMMARY');
...
importWashingtonData().catch(console.error);