...
async function importAlaskaData() {
  console.log('Starting Alaska facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('alaska_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Alaska facilities data files found.');
    return;
  }
  ...
  for (const facility of data) {
    try {
      ...
      imported++;
      console.log(`✓ Imported: ${facility.name} - ${facility.city}, ${facility.county} Borough`);
  ...
  console.log('\n' + '='.repeat(50));
  console.log('ALASKA IMPORT SUMMARY');
...
importAlaskaData().catch(console.error);