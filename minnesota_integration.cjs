...
async function importMinnesotaData() {
  console.log('Starting Minnesota facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('minnesota_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Minnesota facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('MINNESOTA IMPORT SUMMARY');
...
importMinnesotaData().catch(console.error);