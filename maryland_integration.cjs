...
async function importMarylandData() {
  console.log('Starting Maryland facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('maryland_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Maryland facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('MARYLAND IMPORT SUMMARY');
...
importMarylandData().catch(console.error);