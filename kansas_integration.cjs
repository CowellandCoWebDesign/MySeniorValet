...
async function importKansasData() {
  console.log('Starting Kansas facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('kansas_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Kansas facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('KANSAS IMPORT SUMMARY');
...
importKansasData().catch(console.error);