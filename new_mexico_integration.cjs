...
async function importNewMexicoData() {
  console.log('Starting New Mexico facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('new_mexico_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No New Mexico facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('NEW MEXICO IMPORT SUMMARY');
...
importNewMexicoData().catch(console.error);