...
import { Star, Shield, AlertTriangle, DollarSign, MapPin, Heart, Share, Users, Calendar, CheckCircle, ExternalLink, Clock, Home, Wifi, Car, Utensils, Activity, Phone, Camera, Video, UserCheck, Stethoscope, Bed, ShowerHead, ChevronDown, ChevronUp, ImageIcon, ShieldCheck, Circle } from "lucide-react";
      ...
      <CardContent className="p-6 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
        ...
        <div className="flex justify-between items-start mb-4 gap-4">
          <div className="flex-1 min-w-0 max-w-[calc(100%-140px)]">
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2 hover:text-blue-600 transition-colors duration-300 truncate">{community.name}</h3>
            ...
            <div className="flex items-center text-gray-600 dark:text-gray-400 text-sm mb-3 hover:text-gray-800 dark:text-gray-200 transition-colors duration-200">
              ...
              {community.address}, {community.state}
            </div>
          </div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4 hover:bg-slate-100 transform hover:scale-[1.01] transition-all duration-300">
          <div className="flex items-center space-x-2 mb-3">
            <CheckCircle className="h-5 w-5 text-slate-600" />
            <span className="font-bold text-slate-900 text-lg">Key Services:</span>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-3">
              <Circle 
                className={`h-3 w-3 ${community.careTypes?.includes('Skilled Nursing') || community.careTypes?.includes('Memory Care') || community.careTypes?.includes('Assisted Living') 
                  ? 'text-green-500 fill-green-500' 
                  : 'text-red-500 fill-red-500'
                }`} 
              />
              <span className="text-sm text-slate-700">24/7 Medical Staff</span>
            </div>

            {/* Medication Management */}
            <div className="flex items-center space-x-3">
              <Circle 
                className={`h-3 w-3 ${community.careTypes?.includes('Assisted Living') || community.careTypes?.includes('Memory Care') || community.careTypes?.includes('Skilled Nursing')
                  ? 'text-green-500 fill-green-500' 
                  : 'text-red-500 fill-red-500'
                }`} 
              />
            ...
            <div className="flex items-center space-x-3">
              <Circle 
                className={`h-3 w-3 ${community.amenities?.some(amenity => 
                  amenity.toLowerCase().includes('housekeeping') || 
                  amenity.toLowerCase().includes('cleaning') ||
                  amenity.toLowerCase().includes('maintenance')
                ) || community.careTypes?.includes('Assisted Living') || community.careTypes?.includes('Memory Care')
                  ? 'text-green-500 fill-green-500' 
                  : 'text-red-500 fill-red-500'
                }`} 
              />
              <span className="text-sm text-slate-700">Housekeeping Included</span>
            </div>

            {/* Transportation Included */}
            <div className="flex items-center space-x-3">
              <Circle 
                className={`h-3 w-3 ${community.transportationServices?.length > 0 || 
              ...
              />
              <span className="text-sm text-slate-700">Transportation Included</span>
            </div>
          </div>
