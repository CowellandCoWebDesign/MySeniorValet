...
export default function MapSearch() {
  ...
  const panelParam = urlParams.get('panel') as 'messages' | 'tours' | 'compare' | 'none' | null;
  ...
  // Panel state for inline overlays (Zillow-style)
  const [activePanel, setActivePanel] = useState<'none' | 'messages' | 'tours' | 'compare'>(
    panelParam && ['messages', 'tours', 'compare'].includes(panelParam) ? panelParam : 'none'
  );
  ...
  // Update URL parameter when panel changes
  useEffect(() => {
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col bg-gray-50 dark:bg-gray-800 pb-16 md:pb-0">
      ...
      {/* Inline Panel Sheets (Zillow-style) */}
      {/* Messages Panel */}
      <Sheet open={activePanel === 'messages'} onOpenChange={(open) => setActivePanel(open ? 'messages' : 'none')}>
      ...
      {/* Tours Panel */}
      <Sheet open={activePanel === 'tours'} onOpenChange={(open) => setActivePanel(open ? 'tours' : 'none')}>
        <SheetContent side="right" className="w-full sm:max-w-4xl overflow-y-auto">
          <div className="h-full overflow-y-auto">
            <ToursPage />
          </div>
      ...
      {/* Compare Panel */}
      <Sheet open={activePanel === 'compare'} onOpenChange={(open) => setActivePanel(open ? 'compare' : 'none')}>
