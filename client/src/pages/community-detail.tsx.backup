...
export default function CommunityDetail() {
  ...
  const nextPhoto = () => {
    if (community?.photos && community.photos.length > 0) {
      setCurrentPhotoIndex((prev) => (prev + 1) % community.photos.length);
    }
  };

  const prevPhoto = () => {
    if (community?.photos && community.photos.length > 0) {
      setCurrentPhotoIndex((prev) => (prev - 1 + community.photos.length) % community.photos.length);
    }
  };
            ...
            <Card className="overflow-hidden">
              <div className="relative">
                {community.photos && community.photos.length > 0 ? (
                  <div className="relative">
                    <img
                      src={community.photos[currentPhotoIndex]}
                      alt={`${community.name} - Photo ${currentPhotoIndex + 1}`}
                      className="w-full h-96 object-cover"
                    />
                    
                    {/* Favorites Heart Button */}
                    <button className="absolute top-4 right-4 bg-white/90 hover:bg-white text-gray-700 p-2 rounded-full shadow-md transition-colors">
                    ...
                    <div className="absolute bottom-4 right-4 bg-black/50 text-white px-3 py-1 rounded-full text-sm">
                      {currentPhotoIndex + 1} / {community.photos.length}
                    </div>
                  </div>
                ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      {/* Certifications */}
                      <div>
                        <h3 className="text-sm font-medium text-gray-700 mb-2">Certifications & Awards</h3>
                        <div className="flex flex-wrap gap-2">
                          <Badge className="bg-blue-100 text-blue-800 border border-blue-300">
