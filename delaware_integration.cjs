...
async function importDelawareData() {
  console.log('Starting Delaware facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('delaware_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Delaware facilities data files found.');
    return;
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('DELAWARE IMPORT SUMMARY');
...
importDelawareData().catch(console.error);