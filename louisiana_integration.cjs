...
async function importLouisianaData() {
  console.log('Starting Louisiana facilities data import...\n');
  
  const sql = neon(process.env.DATABASE_URL);
  
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith('louisiana_complete_facilities_') && 
    f.endsWith('.json')
  );
  
  if (files.length === 0) {
    console.error('No Louisiana facilities data files found.');
    return;
  }
  ...
  for (const facility of data) {
    try {
      ...
      imported++;
      console.log(`✓ Imported: ${facility.name} - ${facility.city}, ${facility.parish} Parish`);
  ...
  console.log('\n' + '='.repeat(50));
  console.log('LOUISIANA IMPORT SUMMARY');
...
importLouisianaData().catch(console.error);